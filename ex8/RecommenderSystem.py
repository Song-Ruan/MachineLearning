...
def computeGrad(params,Y,R,nm,nu,nf,l=0):
    ...
    X, Theta = deserialize(params, nm, nu, nf)
    dx=((X.dot(Theta.T)-Y)*R).dot(Theta)+1*X#记住求grad的时候千万不要sum，因为此时期待得出的是nm*nf的矩阵而不是一个值
    dtheta=((X.dot(Theta.T)-Y)*R).T.dot(X)+1*Theta
...
res=opt.minimize(fun=computeCost,x0=params,args=(Y,R,nm,nu,nf,l),method='TNC',jac=computeGrad,options={'maxiter':100})
ret=res.x
